...
import { Button, Text, View, StyleSheet, FlatList, RefreshControl, ScrollView } from "react-native";
    ...
    render(){
        ...
        return (
            ...
            <FlatList
                ...
                data={users}
                renderItem={({ item }) => <Item title={`ID\t\t\t\t\t\t:\t\t${item.id}\nName\t\t\t:\t\t${item.first_name}\nEmail\t\t\t:\t\t${item.email}\nAvatar\t\t:\t\t${item.avatar}`}/>}
...
export default connect(mapStateToProps, matchDispatchToProps)(UsersScreen);
